...
void TestConnectorMetadata::addTable(
    ...
    const RowTypePtr& schema,
    uint64_t rows) {
  auto table = std::make_unique<TestTable>(name, schema, connector_, rows);
  auto [_, emplaced] = tables_.emplace(name, std::move(table));
  VELOX_CHECK(emplaced, "table {} already exists", name);
}
...
void TestConnector::addTable(
    ...
    const RowTypePtr& schema,
    uint64_t rows) {
  metadata_->addTable(name, schema, rows);
}
